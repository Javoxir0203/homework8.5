...
function HeaderPage({ headerText, headerLogo, logo, signUp }) {
			...
			<div className='lg:w-[85%] w-[100%] mx-auto flex items-center justify-between mt-6'>
				...
				<div className='flex items-center gap-3 md:gap-5 justify-end'>
					<span className='cursor-pointer font-[400] text-[16px] hover:bg-[#F1F1F3] px-[12px] rounded-[8px] py-[14px] transition ease-in-out duration-500'>
						<NavLink to='/Sign Up' className={({ isActive }) => (isActive ? activeLink : normalLink)}>
							{signUp}
						</NavLink>
					</span>
					<NavLink to='/Log In'>
						<Button label='Login' type='button' styles='bg-[#FF9500] py-[14px] px-[24px] md:px-[34px] rounded-[8px] text-white font-[400] text-[16px]' />
					</NavLink>
					<Bars navs={navs} />
				</div>
			</div>
...
function Bars({ navs }) {
	...
	return (
		<div className={`barsSection lg:hidden`}>
			<FontAwesomeIcon icon={isBarsClicked ? faXmark : faBars} className='text-[30px] cursor-pointer lg:hidden' onClick={handleToggle} />
