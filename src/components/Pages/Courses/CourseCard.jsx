...
function CourseCard({ card }) {
	const { courseTrack, courseDes, courseImages, courseDurationStatus, courseTrainer, courseProcess, semiCards } = card;
			...
			<div className='bg-white rounded-[12px] p-[20px] md:p-[40px]' data-aos='zoom-in' data-aos-duration='1000' data-aos-once='true'>
				<div>
					<div className='flex flex-col lg:flex-row lg:items-center'>
						...
						<Button type='button' label='View Courses' styles='border border-[#F1F1F3] bg-[#FCFCFD] rounded-[8px] py-[14px] px-[14px] font-[500] text-[16px] text-[262626] w-[140px] lg:ms-[200px] mt-0 mb-[30px] lg:mt-0' />
					</div>
					<div className='grid grid-cols-3 gap-[10px] md:gap-[30px] relative overflow-hidden rounded-[6px]'>
						{courseImages.map(courseImage => {
							return <img key={courseImage} src={courseImage} className='rounded-[6px] imgScale' aria-label='lazy' decoding='async' />;
						})}
					</div>
					<div className='flex flex-col md:flex-row md:justify-between md:items-center pb-[50px]'>
						<div className='flex gap-3 pt-[25px]'>
							{courseDurationStatus.map(eachLevel => {
								return (
									<span key={eachLevel} className='text-[#59595A] text-[16px] font-[400] border border-[#F1F1F3] rounded-[8px] py-[10px] px-[16px]'>
										{eachLevel}
									</span>
								);
							})}
