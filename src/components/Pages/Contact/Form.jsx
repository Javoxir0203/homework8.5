				...
				<div className='bg-white rounded-[8px] p-[20px] md:p-[50px]'>
					<form id='myForm' onSubmit={handleSubmit}>
						<div className='grid-wrapper'>
							<div>
								<Input label='First Name' labelFor='firstname' inputType='text' placeholder='Enter first name' inputName='firstname' value={formData.firstname} onChange={handleChange} onBlur={handleBlur} error={errors.firstname} />
								<span className='text-[12px] text-[red] font-[500]'>{errors.firstname}</span>
							</div>
							<div>
								<Input label='Last Name' labelFor='lastname' inputType='text' placeholder='Enter last name' inputName='lastname' value={formData.lastname} onChange={handleChange} onBlur={handleBlur} error={errors.lastname} />
								<span className='text-[12px] text-[red] font-[500]'>{errors.lastname}</span>
							</div>
							<div>
								<Input label='Email' labelFor='email' inputType='text' placeholder='Enter your email address' inputName='email' value={formData.email} onChange={handleChange} onBlur={handleBlur} error={errors.email} />
								<span className='text-[12px] text-[red] font-[500]'>{errors.email}</span>
							</div>
							<div>
								<Input label='Phone Number' labelFor='phonenumber' inputType='text' placeholder='Enter your phone number' inputName='phonenumber' value={formData.phonenumber} onChange={handleChange} onBlur={handleBlur} error={errors.phonenumber} />
								<span className='text-[12px] text-[red] font-[500]'>{errors.phonenumber}</span>
							</div>
							<div className='last--Item'>
								<Input label='Subject' labelFor='subject' inputType='text' placeholder='Enter subject' inputName='subject' value={formData.subject} onChange={handleChange} onBlur={handleBlur} error={errors.subject} />
								<span className='text-[12px] text-[red] font-[500]'>{errors.subject}</span>
							</div>
							<div className='item1'>
								<TextArea areaName='message' id='message' labelFor='message' label='Message' placeholder='Enter your Message here.....' value={formData.message} onChange={handleChange} onBlur={handleBlur} error={errors.message} />
								<span className='text-[12px] text-[red] font-[500]'>{errors.message}</span>
							</div>
						</div>
						<div className='flex justify-center pt-[35px]'>
							<Button type='submit' label='Send Your Message' styles='bg-[#FF9500] py-[14px] px-[20px] rounded-md text-[14px] font-[500] text-white' />
						</div>
					</form>
				</div>
			</div>
						...
						<div class='w-full flex justify-center'>
							<div class='mark-wrapper'>
								<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100' viewBox='0 0 151 150' fill='none'>
									...
									<path d='M109.167 52L63.3333 97.8334L42.5 77' stroke='#FF9500' stroke-width='6' stroke-linecap='round' stroke-linejoin='round' />
								</svg>
							</div>
						</div>
