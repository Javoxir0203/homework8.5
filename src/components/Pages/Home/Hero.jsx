...
function Hero({ spark, sparkAlt, flashIcon, flashIconAlt, subHeroTitle, heroNtitle, heroText, heroTitle, heroImg, heroAlt }) {
	const brands = [
		{
		...
		<div className='w-[95%] md:w-[90%] lg:w-[80%] mx-auto flex justify-center mt-[100px]'>
			<div className='w-[100%]'>
				...
				<div className='flex justify-center items-center gap-2 mt-[50px]'>
					<NavLink to='./courses'>
						<Button label='Explore Courses' type='button' styles='font-[600] text-[16px] py-[18px] px-[24px] rounded-[8px] bg-[#FF9500] text-white hover:bg-[#ff9500f3] transition ease-in-out duration-200' />
					</NavLink>
					<NavLink to='./pricing'>
						<Button label='View Pricing' type='button' styles='font-[600] text-[16px] py-[18px] px-[24px] rounded-[8px] bg-[#fff] border border-[#F1F1F3]' />
					</NavLink>
				</div>
				<div className='border border-[#F1F1F3] bg-[#FCFCFD] rounded-[12px] flex justify-center gap-[60px] items-center px-[30px] py-[50px] my-[90px] flex-wrap  brand'>
					{brands.map(brand => {
						return <img src={brand.image} alt={brand.imageAlt} key={brand.imageAlt} aria-label='lazy' decoding='async' />;
					})}
				</div>
				<div>
