	...
	return (
		<div className='w-[95%] md:w-[90%] lg:w-[80%] mx-auto'>
			<div className='flex flex-col lg:flex-row lg:items-center'>
				<Title title='Our Courses' subTitle='Lorem ipsum dolor sit amet consectetur. Tempus tincidunt etiam eget elit id imperdiet et. Cras eu sit dignissim lorem nibh et. Ac cum eget habitasse in velit fringilla feugiat senectus in.' />
				<Button label='View All' type='button' styles='border border-[#F1F1F3] bg-[#FCFCFD] rounded-[8px] py-[14px] px-[24px] font-[500] text-[16px] text-[262626] w-[130px] lg:ms-[200px] mt-6 lg:mt-0  hover:bg-[#eaeaeb] transition ease-in-out duration-500' />
			</div>
			<div className='grid grid-cols-1 lg:grid-cols-2 gap-[30px] my-[70px]'>
				{tracks.map(track => {
					return <CoursesCard key={track.id} courseCard={track} />;
				})}
			</div>
		</div>
	);
...
function CoursesCard({ courseCard }) {
	const { image, imageAlt, cardDuration, cardLevel, cardName, cardTrack, cardDescrption } = courseCard;
		...
		<div>
			<div className='bg-[#fff] border border-[#F1F1F3] rounded-[12px] p-[15px] md:p-[30px] course-card overflow-hidden' data-aos='zoom-in' data-aos-duration='1000' data-aos-once='true'>
				<div>
					...
					<div>
						<NavLink to='./courses'>
							<Button label='Get it Now' type='button' styles='border border-[#F1F1F3] rounded-[8px] bg-[#F7F7F8] w-[100%] py-[12px] font-[500] text-[15px] hover:bg-[#eaeaeb] transition ease-in-out duration-500' />
						</NavLink>
					</div>
				</div>
			</div>
