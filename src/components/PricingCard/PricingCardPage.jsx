...
function PricingCardPage({ card }) {
	const { label, num, status, date, benefits } = card;
			...
			<div className='border border-[#F1F1F3] bg-[#FCFCFD] rounded-[12px] py-[20px] px-[10px] md:py-[40px] md:px-[25px]' data-aos='zoom-in' data-aos-easing='ease-in-out' data-aos-once='true' data-aos-duration='1000'>
				...
				<div className='flex justify-center my-5'>
					<span className='text-[60px] font-[700]'>
						{num}
						<span className='font-[500] text-[16px]'>{date}</span>
					</span>
				</div>
				<div className='border border-[#F1F1F3] rounded-[14px] pt-[20px] bg-white'>
					...
					<NavLink to='/contact'>
						<Button label='Get Started' type='button' styles='bg-[#FF9500] py-[20px] rounded-bl-[8px] rounded-br-[8px] w-[100%] text-white text-[15px] font-[600] hover:bg-[#ff9500f3] transition ease-in-out duration-200' />
					</NavLink>
				</div>
			</div>
		</div>
